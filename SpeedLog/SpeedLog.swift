...
//Helper
  public static var AllOptions: LogMode  { return FileName.union(FuncName).union(Line) }
}
...
private extension SpeedLog {
  static func printStringForMode<T>(object: T, file: String, function: String, line: Int) -> String {
    ...
    if mode.intersect(.FileName) != [] {
        
      let filename = file.lastPathComponent.stringByDeletingPathExtension
      result = "\(filename)."
    }
    if mode.intersect(.FuncName) != [] {
      result += "\(function)"
    }
    if mode.intersect(.Line) != [] {
      result += "[\(line)]"
    }
